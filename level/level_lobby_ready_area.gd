# Export variable packed scene of target level to load
...
@onready var target_level_name = target_level.resource_path.get_file().get_basename()

signal player_ready(target_level_name, player_body, ready_status)
...
func _on_player_ready(player_body: Node, ready_status: bool) -> void:
	if multiplayer.is_server():
		emit_signal("player_ready", target_level_name.get_basename(), player_body, ready_status)
